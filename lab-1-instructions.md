...
<div class="function-category">
<h3>Cell Reference Types</h3>

<div class="function-card">
<h4>Relative References</h4>
**Format:** `A1`, `E2`, `F10`

**Behavior:** When copied, the row and column adjust relative to the new location.

**Example:**
- Original formula in J2: `=E2*F2`
- Copied to J3: becomes `=E3*F3`
- Copied to J4: becomes `=E4*F4`

**Use When:**
- Formula should adjust for each row/column
- Calculating values for multiple items
...
**Behavior:** When copied, the row and column remain locked (don't change).

**Example:**
- Original formula in M2: `=E2-$B$3`
- Copied to M3: becomes `=E3-$B$3` (E3 changes, $B$3 stays)
- Copied to M4: becomes `=E4-$B$3` (E4 changes, $B$3 stays)

**Use When:**
- Referencing a fixed value (like average, tax rate, constant)
- Comparing all values to a single reference point
- Creating formulas with fixed denominators or multipliers
...
<div class="function-card">
<h4>Mixed References</h4>
**Format:** `$A1` (column locked), `A$1` (row locked)

**Behavior:** One part is locked, the other adjusts.

**Examples:**
- `$A1`: Column A locked, row adjusts when copied down
- `A$1`: Row 1 locked, column adjusts when copied right
- `$A$1*B$1`: Column A locked, row 1 locked, but column B adjusts

**Use When:**
- Creating tables where one dimension is fixed
- Multiplication tables
- Complex structured calculations

**How to Toggle Reference Types:**
- **Excel/Google Sheets**: Select cell reference in formula bar, press F4 to cycle: A1 → $A$1 → A$1 → $A1 → A1
